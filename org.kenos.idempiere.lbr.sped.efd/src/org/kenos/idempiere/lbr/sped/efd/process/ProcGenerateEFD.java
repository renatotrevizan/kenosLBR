 ...
 *	
 *	Baseado nas contribuições do Pablo Boff (RoundIT) e Mario Grigioni (Kenos, Eltek)
 * 
...
public class ProcGenerateEFD extends SvrProcess
{
	...
	protected String doIt () throws Exception
	{
		...
		//	SPED
		MLBRSPED sped = new MLBRSPED (getCtx(), p_LBR_SPED_ID, get_TrxName());
		
		// 	Validations
		if (sped.getC_Period_ID() == 0)
			throw new IllegalArgumentException("@C_Period_ID@ @Mandatory@");
		
		if (sped.getAD_Org_ID() == 0)
		...
		List <MLBRTaxAssessment> list = new Query(Env.getCtx(), 
											...
											get_TrxName())
										.setParameters(Env.getAD_Client_ID(getCtx()), sped.getAD_Org_ID(), sped.getC_Period_ID())
										.list();
		 ...
		 * Rodar Processo
		 */
		StringBuilder result = generateEFD (sped);
		 ...
		 * EDF_CNPJ_DATA.txt
		 */
		I_W_AD_OrgInfo oi = POWrapper.create(MOrgInfo.get(getCtx(), sped.getAD_Org_ID(), get_TrxName()), I_W_AD_OrgInfo.class);
		String fileName = "EFD_" + TextUtil.toNumeric(oi.getlbr_CNPJ()) + "_" + TextUtil.timeToString(dateFrom, "MMyyyy") + ".txt";
	 ...
	 * @throws Exception
	 */
	private StringBuilder generateEFD (MLBRSPED sped) throws Exception
	{
		List<MLBRFactFiscal> facts = MLBRFactFiscal.get (getCtx(), sped.getDateFrom(), sped.getDateTo(), sped.getAD_Org_ID(), null, null); 

		MLBRSalesCardTotal[] cards = MLBRSalesCardTotal.get(getCtx(), sped.getC_Period_ID(), null);
		
		Reg0000 reg0000 = new Reg0000();
		reg0000.setCodVer(14);						//	FIXME
		reg0000.setCodFin(0);						//	FIXME
		reg0000.setDtIni(sped.getDateFrom().toLocalDateTime().toLocalDate());
		reg0000.setDtFin(sped.getDateTo().toLocalDateTime().toLocalDate());
		reg0000.setNome(sped.getlbr_LegalEntity());
		reg0000.setCnpj(sped.getCNPJ());
		reg0000.setUf(sped.getUF());
		reg0000.setIe(sped.getIE());
		reg0000.setCodMun(sped.getCityCode());
		reg0000.setIm(sped.getIM());
		reg0000.setSuframa(sped.getSUFRAMA());
		...
		reg0005.setCep(sped.getPostal());
		reg0005.setEnd(sped.getAddress1());
		reg0005.setNum(sped.getAddress2());
		reg0005.setCompl(sped.getAddress4());
		reg0005.setBairro(sped.getAddress3());
